...
{
    /// <summary>
    /// 인벤토리의 몇 번째 슬롯과 연결되었는지. 몇번째 슬롯인지.
    /// </summary>
    uint id;
    public uint ID => id;
    ...
    /// 슬롯 초기화용 함수
    /// </summary>
    /// <param name="id">슬롯 인덱스. 슬롯ID의 역할도 함</param>
    ...
    public virtual void InitializeSlot(uint id, ItemSlot slot)
    {
        ...
        this.id = id;       //값 설정
        itemSlot = slot;
        itemSlot.onSlotItemChange = Refresh;    //슬롯에 들어있는 아이템이 변경되었을 때 실행 될 함수등록
    ...
    private void Refresh()
    {
        if(itemSlot.IsEmpty)
        {
            ...
            itemImage.color = Color.clear;  //color에는 알파값 조절이 없다.
            itemCount.text = string.Empty;  //갯수 비우기
        
        }   
        else
        {
            //슬롯에 아이템이 들어있을 때
            itemImage.sprite = itemSlot.ItemData.itemIcon;      //이미지 설정하기
